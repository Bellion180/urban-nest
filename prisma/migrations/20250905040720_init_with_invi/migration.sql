...
CREATE TABLE `users` (
    ...
    `role` ENUM('ADMIN', 'USER', 'RESIDENT') NOT NULL DEFAULT 'RESIDENT',
...
CREATE TABLE `residents` (
    ...
    `deudaActual` DOUBLE NOT NULL DEFAULT 0.00,
    `pagosRealizados` DOUBLE NOT NULL DEFAULT 0.00,
...
-- CreateTable
CREATE TABLE `invi_info` (
    `id` VARCHAR(191) NOT NULL,
    `idInvi` VARCHAR(191) NULL,
    `mensualidades` INTEGER NULL,
    `fechaContrato` DATETIME(3) NULL,
    `deuda` DOUBLE NOT NULL DEFAULT 0.00,
    `idCompanero` VARCHAR(191) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `residentId` VARCHAR(191) NOT NULL,

    UNIQUE INDEX `invi_info_residentId_key`(`residentId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `invi_info` ADD CONSTRAINT `invi_info_residentId_fkey` FOREIGN KEY (`residentId`) REFERENCES `residents`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
