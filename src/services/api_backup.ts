...
export const residentService = {
  ...
  updateWithDocuments: async (id: string, formData: FormData) => {
    ...
    console.log(`API: Making multipart request to /residents/${id}/documents`);
    ...
    console.log(`API: Response from /residents/${id}/documents:`, {
      status: response.status,
      statusText: response.statusText,
      ok: response.ok,
    ...
    if (!response.ok) {
      ...
      try {
        if (contentType && contentType.includes('application/json')) {
          ...
          // Si no es JSON, obtener texto plano
          const errorText = await response.text();
          errorDetails = { message: `Error ${response.status}: ${errorText}` };
        }
        console.error(`API: Error details from /residents/${id}/documents:`, errorDetails);
      } catch (parseError) {
        errorDetails = { message: `Error de red: ${response.status} ${response.statusText}` };
        console.error(`API: Could not parse error from /residents/${id}/documents:`, parseError);
      }
  ...
  // Realizar pago de un residente
  makePayment: async (residentId: string, paymentData: {
    amount: number;
    paymentDate: string;
    description?: string;
  }) => {
    const response = await authenticatedFetch(`/residents/${residentId}/payment`, {
      method: 'POST',
